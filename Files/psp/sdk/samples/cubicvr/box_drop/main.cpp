...
*/
	...
	if (!myTimer.paused()) scene.getDynamicsWorld()->stepSimulation(lus, 10);
}
...
void CreateTower(int height, XYZ position)
{
	for(int i=0; i<height; i++)
	{
		DropBox(position + XYZ(0,(i*5),0));
	}
}

void CreateStack(int width, int height, int depth, XYZ position)
{
	for(int z=0; z<depth; z++)
		for(int y=0; y<height; y++)
			for(int x=0; x<width; x++)
			{
				DropBox(position + XYZ((y*5),(x*5),(z*5)));
			}
}
...
//		pspDebugScreenSetXY(0,1);
	   ...
	   // create a dynamics dropping box
		...
		{
//			CreateStack(3, 3, 3, XYZ(0,6,0));
						CreateStack(1, 1, 1, XYZ(0,6,0));
			pad_lock = 0;
		}
...
//		camera.trackTarget(1.0*lus,safeDistance);
		
		// lower height clamp
		if (camera.position.y < safeDistance) camera.position.y = safeDistance;
		// upper height clamp
		if (camera.position.y > safeDistance+5.0) camera.position.y = safeDistance+5.0;
		
//		camera.moveViewRelative(camera.target,(pad.Lx / 128.0)*5.0*lus,(pad.Ly / 128.0)*5.0*lus);



		// camera.position += XYZ((pad.Lx / 128),0,(pad.Ly / 128));
		// camera.target += XYZ(pad.Lx / 128,0,pad.Ly / 128);
